...
ax1.set_ylabel("Percentage of the call to gpuMap spent on \n moving memory to device + kernel execution + moving result from device, \n the rest is Java overhead", fontsize=7)
...
pp = PdfPages(os.path.join(write_to, sys.argv[1].split('.')[0] + '_gpu_instr_graph.pdf'))
